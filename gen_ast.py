...
def define_ast(output_file: FileIO, base_name: str, types: List[str]):
    ...
    for type in types:
        ...
        output_file.write(
            f"            return visitor.visit_{class_name}(self)\n")
...
def main():
    # Generate an lox_ast.py file in a command line argument <output directory>.
    import argparse
    import sys
    import os
    argparser = argparse.ArgumentParser(
        description='Generate a lox_ast.py file in a command line argument <output directory>.')
    ...
    if not os.path.isdir(args.output_directory):
        ...
        sys.exit(1)
    # Generate the lox_ast.py file.
    output_file = open(os.path.join(args.output_directory, 'lox_ast.py'), 'w')
